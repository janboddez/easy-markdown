...
class WPCom_Markdown {
	...
	public function is_posting_enabled() {
		...
		return get_option( self::POST_OPTION, false );
	}
	...
	public function is_commenting_enabled() {
		...
		return get_option( self::COMMENT_OPTION, false );
	}
	...
	public function get_parser() {
		...
		if ( ! self::$parser ) {
			require dirname( __FILE__ ) . '/markdown/extra.php';
			require dirname( __FILE__ ) . '/markdown/gfm.php';
...
jQuery( function() {
	...
	public function transform( $text, $args = array() ) {
		// phpcs:disable Squiz.Commenting.InlineComment.SpacingBefore,Squiz.Commenting.InlineComment.InvalidEndChar
	...
	protected function fix_latest_revision_on_restore( $post_id ) {
		...
		$last_revision                        = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE post_type = 'revision' AND post_parent = %d ORDER BY ID DESC", $post->ID ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
